    ...
    I2C: I2c,
{
    ...
    pub fn init_open_loop_erm(&mut self) -> Result<(), I2C::Error> {
        ...
        control3.set_erm_open_loop(false);
    ...
    pub fn calibrate(&mut self, value: u8) -> Result<(), I2C::Error> {
        // V = 21.59 × 10e-3 OD_CLAMP[7:0]
        self.write(Register::Mode, 7)?;

        let mut feedback = FeedbackControlReg(self.read(Register::FeedbackControl)?);
        feedback.set_fb_brake_factor(2);
        feedback.set_loop_gain(2);
        self.write(Register::FeedbackControl, feedback.0)?;

        let mut control4 = Control4Reg(self.read(Register::Control4)?);
        control4.set_auto_cal_time(4);
        self.write(Register::Control4, control4.0)?;

        self.write(Register::OverdriveClampVoltage, value)?;
        self.write(Register::RatedVoltage, value)?;
